...
const processCSV = (orderObjArray) => {
    ...
    orderObjArray.forEach((dirtyOrderObj, x)=> {
        if (x > 0) {
            ...
            if (orderObj.OrderDate) {
                ...
                // group by category
                if (!tmpByCategoryObj[orderObj.Category]) {
                    tmpByCategoryObj[orderObj.Category] = {
                        total: currency(0),
                        records: []
                    }
                }

                tmpByCategoryObj[orderObj.Category].records.push(orderObj.OrderID);
                tmpByCategoryObj[orderObj.Category].total = tmpByCategoryObj[orderObj.Category].total.add(orderObj.ItemTotal);
    ...
    // sort categories
    outputObj.byCategory = tmpByCategoryObj;
